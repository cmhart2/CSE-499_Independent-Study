...
void SenderTask(void* pvParameters) {
	...
	while(1) {
		...
		printf("pdPASS == qStatus: %i, pdTRUE == qStatus: %i", (qStatus == pdPASS), (qStatus == pdTRUE));
		if (qStatus != pdPASS) {
			printf("Error: Data could not be sent \t qStatus = %li \r\n", qStatus);
		}
...
void ReceiverTask(void* pvParameters) {
	...
	BaseType_t qStatus;
	const TickType_t waitTime = pdMS_TO_TICKS(100); // 100ms wait time
	...
	while(1) {
		...
		if (qStatus == pdPASS) {
			printf("The value received is: %li \r\n", valueReceived);
		}
		else {
			printf("Error: Data could not be received \t qStatus = %li \r\n", qStatus);
		}
	}
}
