...
void vBlueLEDcontrollerTask(void *pvParameters) {
	while(1){
		BlueTaskProfiler++;
		vTaskPrioritySet(NULL, 1); // NULL -> we are already in the blue_handler, so we don't have to say "blue_handler"
	};
}
